...
export const tournaments = (await getCollection('tournaments')).sort((a, b) => 
...
{tournaments.length === 0 ? (
  ...
  <ul>
    {tournaments.map((tournament) => (
      <li>
        <a href={`/events/tournaments/${tournament.slug}`}>
          <strong>{tournament.data.title}</strong>
        </a>
        {' - '}
        {tournament.data.date.toLocaleDateString('en-AU', { 
          year: 'numeric', 
          month: 'long', 
          day: 'numeric' 
        })}
